export function calculateElo(computers, tasks, distribution, results) {
  ...
  const computerToResults = results.reduce((map, result) => {
    ...
    const computer = Object.keys(distribution).find(key => {
      const match = Object.values(distribution[key]).find(distributedTask => distributedTask.taskId === task.taskId);
  ...
  return computers.map(computer => {
    ...
    const averageResult = results.reduce((acc, result) => {
      acc.executionTime += result.executionTime;
      return acc;
    }, { executionTime: 0 });

    let updatedElo = {
      execution: computer.elo.execution + averageResult.executionTime / results.length
    };

    updatedElo.execution = isNaN(updatedElo.execution) ? 0 : updatedElo.execution;
    return {
      ...computer,
      elo: updatedElo
    };
...
export function calculateNascar(computers, tasks, distribution, results) {
  ...
  computersWithResults.sort((a, b) => {
    const aAvg = a.results.reduce((acc, result) => acc + result.executionTime, 0) / a.results.length;
    const bAvg = b.results.reduce((acc, result) => acc + result.executionTime, 0) / b.results.length;
    return aAvg - bAvg;
  });

  return computers.map(computer => {
    const computerWithResults = computersWithResults.find(c => c.name === computer.name);
    let updatedNascar = {};
    ...
    updatedNascar.finish = isNaN(updatedNascar.finish) ? 0 : updatedNascar.finish;
    return {
      ...computer,
